import { AlertTriangle, Edit, Eye, Info, Plus, XCircle } from "lucide-react";
  ...
  return (
    <div data-testid="field-card" className={cardClass}>
      <div className="mb-2 flex items-start justify-between gap-2">
        <h4 className="text-xs font-semibold text-gray-900">{field.field_name}</h4>
      ...
      {field.field_state === "no_aplica" ? (
        <div className="mb-2 flex items-center gap-2 text-sky-700">
          <Info className="h-4 w-4" />
          <p className="text-sm font-medium">No aplica para este pliego</p>
        </div>
      ) : null}
      ...
      {field.field_state === "en_conflicto" ? (
        <div className="mb-2 space-y-2">
          ...
          {field.citations.map((citation, index) => (
            <div key={`${citation.document_id}-${index}`} className="rounded border border-error/30 bg-white p-1.5">
      ...
      {primaryCitation ? (
        <div className="mb-2 rounded bg-gray-50 p-1.5 text-xs text-gray-600">
