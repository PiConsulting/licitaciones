...
def test_plazo_fecha_iso_valid(valid_source_ref):
    ...
    plazo = PlazoItem(
        ...
        hora="10:00",
        texto_original="Las ofertas se recibirán hasta el 15/08/2026 a las 10:00 hs.",
        confidence=0.9,
        source_references=[valid_source_ref],
    )
...
def test_plazo_fecha_argentina_format_fails(valid_source_ref):
    ...
    with pytest.raises(ValidationError) as exc_info:
        PlazoItem(
            ...
            texto_original="Las ofertas se recibirán hasta el 15/08/2026.",
            confidence=0.9,
            source_references=[valid_source_ref],
        )
...
def test_plazo_fecha_none_valid(valid_source_ref):
    ...
    plazo = PlazoItem(
        ...
        texto_original="Las consultas podrán realizarse hasta 10 días corridos desde la apertura.",
        confidence=0.8,
        source_references=[valid_source_ref],
    )
...
def test_plazo_sin_referencia_falla(valid_source_ref):
    """`referencia` es obligatoria (FIX 2026-08-22: reemplaza al viejo enum
    `tipo`, que se eliminó -- ver PlazoItem en schemas.py)."""
    with pytest.raises(ValidationError):
        PlazoItem(
            fecha="2026-08-15",
            texto_original="Las ofertas se recibirán hasta el 15/08/2026.",
...
def test_objeto_alcance_resumen_valid(valid_source_ref):
    ...
    item = ObjetoAlcanceItem(
        ...
        metadata={"cantidad": 200, "unidad_medida": "resmas"},
        confidence=0.9,
        source_references=[valid_source_ref],
    )
...
def test_requisito_documento_valid(valid_source_ref):
    ...
    requisito = RequisitoAdmisibilidadItem(
        ...
        metadata={"obligatorio": "si", "momento_presentacion": "con_la_oferta", "subsanable": "no"},
        confidence=0.9,
        source_references=[valid_source_ref],
    )
...
def test_criterio_metodo(valid_source_ref):
    ...
    criterio = CriterioEvaluacionItem(
        ...
        metadata={"metodo": "puntaje_ponderado", "puntaje_tecnico_minimo": "70 puntos"},
        confidence=0.9,
        source_references=[valid_source_ref],
    )
...
def test_criterio_factor(valid_source_ref):
    ...
    criterio = CriterioEvaluacionItem(
        tipo="criterio",
        valor="Precio",
        metadata={"tipo_criterio": "precio", "ponderacion_porcentaje": 60.0},
        confidence=0.85,
        source_references=[valid_source_ref],
    )
...
def test_anexo_obligatorio_valid(valid_source_ref):
    ...
    anexo = AnexoObligatorioItem(
        ...
        metadata={"debe_completarse": "si", "debe_firmarse": "si"},
        confidence=0.9,
        source_references=[valid_source_ref],
    )
...
def test_causal_rechazo_formal(valid_source_ref):
    ...
    causal = CausalRechazoItem(
        ...
        metadata={"es_descalificante": "si", "es_subsanable": "no"},
        confidence=0.9,
        source_references=[valid_source_ref],
    )
...
def test_source_references_multiple_valid(valid_source_ref):
    ...
    ref2 = SourceReference(document_id="test-doc-456", page_number=7, citation="b" * 50)
