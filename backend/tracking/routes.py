...
def _http_error_from_code(code: str) -> HTTPException:
    mapping: dict[str, tuple[int, str]] = {
        ...
        "NO_VERSION_YET": (
            ...
            "El análisis todavía no tiene una versión consultable",
        ),
        "FORBIDDEN": (status.HTTP_403_FORBIDDEN, "No tenés permisos para este análisis"),
        "TRACKING_NOT_AVAILABLE": (
            status.HTTP_400_BAD_REQUEST,
            "El seguimiento solo se puede iniciar para análisis finalizados",
        ),
        "TRACKING_NOT_FOUND": (
            status.HTTP_404_NOT_FOUND,
            "Seguimiento no encontrado para el análisis",
        ),
        "TRACKING_CATEGORY_NOT_FOUND": (
            status.HTTP_404_NOT_FOUND,
            "Categoría de seguimiento no encontrada",
        ),
        "TRACKING_ITEM_NOT_FOUND": (status.HTTP_404_NOT_FOUND, "Ítem de seguimiento no encontrado"),
        "TRACKING_ITEM_REQUIRED": (
            status.HTTP_400_BAD_REQUEST,
            "tracking_item_id es obligatorio para comentarios de ítem",
        ),
        "TRACKING_CATEGORY_COMMENT_ONLY": (
            status.HTTP_400_BAD_REQUEST,
            "El comentario siempre debe ser general de la categoría.",
        ),
        "INVALID_TRACKING_TRANSITION": (
            status.HTTP_400_BAD_REQUEST,
            "Transición de estado no permitida",
        ),
        "TRACKING_CATEGORY_CLOSED": (
            status.HTTP_409_CONFLICT,
            "La categoría está cerrada. Reabrí la revisión para modificarla.",
        ),
        "TRACKING_COMPLETED_READ_ONLY": (
            status.HTTP_409_CONFLICT,
            "El seguimiento está finalizado en modo solo lectura.",
        ),
        "TRACKING_COMMENT_NOT_FOUND": (
            status.HTTP_404_NOT_FOUND,
            "Comentario de seguimiento no encontrado",
        ),
        "TRACKING_CONFLICT": (
            status.HTTP_409_CONFLICT,
            "El seguimiento cambió en paralelo. Refrescá el estado y reintentá.",
        ),
    }
    http_status, message = mapping.get(
        code, (status.HTTP_400_BAD_REQUEST, "No se pudo procesar la solicitud de seguimiento")
    )
...
async def start_analysis_tracking(
    analysis_id: str, credentials=Depends(http_bearer)
) -> StartTrackingResponse:
...
async def complete_analysis_tracking(
    analysis_id: str, credentials=Depends(http_bearer)
) -> AnalysisTracking:
    current_user = get_current_user(credentials, None)
    try:
        tracking = complete_tracking(
            analysis_id, current_user.id, completed_by_name=current_user.name
        )
    except (ValueError, PermissionError, RuntimeError) as exc:
        raise _http_error_from_code(str(exc)) from exc
    return AnalysisTracking(**tracking)
...
@tracking_router.patch(
    "/{analysis_id}/tracking/categories/{category_key}/status", response_model=AnalysisTracking
)
async def patch_tracking_category_status(
    ...
    current_user = get_current_user(credentials, None)
    try:
        tracking = update_category_status(
            analysis_id, current_user.id, category_key, payload.status
        )
    except (ValueError, PermissionError, RuntimeError) as exc:
        raise _http_error_from_code(str(exc)) from exc
    return AnalysisTracking(**tracking)
...
@tracking_router.patch(
    "/{analysis_id}/tracking/categories/{category_key}/items/{tracking_item_id}",
    response_model=AnalysisTracking,
)
...
@tracking_router.get(
    "/{analysis_id}/tracking/categories/{category_key}/comments",
    response_model=list[TrackingComment],
)
...
@tracking_router.post(
    "/{analysis_id}/tracking/categories/{category_key}/comments", response_model=TrackingComment
)
...
@tracking_router.patch(
    "/{analysis_id}/tracking/categories/{category_key}/comments/{comment_id}",
    response_model=TrackingComment,
)
