...
from analysis.extraction.glossary import build_keyword_query, get_category_terms, get_category_top_k
...
def test_query_terms_principales():
    ...
    expected_core_terms = ["multa", "penalizacion", "sancion", "incumplimiento", "rescision"]

    for term in expected_core_terms:
        assert any(term in t_lower for t_lower in terms_lower), (
            f"Término '{term}' debe estar en el glossary"
        )
...
def test_aliases_expandidos():
    ...
    for alias in expected_aliases:
        assert any(alias in t_lower for t_lower in terms_lower), (
            f"Alias '{alias}' debe estar en el glossary"
        )
