...
def test_extract_and_index_actualiza_shadow_cosmos_en_etapas_intermedias(
    ...
    monkeypatch.setattr("indexing.runner._build_blob_storage", lambda: _FakeBlobStorage())
    monkeypatch.setattr(
        "indexing.runner.extract_text",
        lambda *_args, **_kwargs: [{"page_number": 1, "content": "contenido pagina"}],
    )
    monkeypatch.setattr(
        "indexing.runner.create_chunks",
    ...
    monkeypatch.setattr(
        "indexing.runner.generate_embeddings",
        lambda chunks, *_args, **_kwargs: [dict(chunk, embedding=[0.1, 0.2]) for chunk in chunks],
    )
    monkeypatch.setattr("indexing.runner.upload_chunks", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "indexing.runner.extract_categories",
...
def test_persist_runtime_state_se_llama_en_indexing_y_analyzing(
    ...
    monkeypatch.setattr("indexing.runner._persist_runtime_state", _fake_persist_runtime_state)
