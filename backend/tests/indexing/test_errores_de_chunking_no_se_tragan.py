...
from indexing.chunking import chunking as chunking_impl
...
def test_un_error_de_codigo_no_se_traga(monkeypatch: pytest.MonkeyPatch, error: type) -> None:
    ...
    monkeypatch.setattr(chunking_impl, "classify_chunk_categories", explota)
...
def test_el_documento_no_queda_indexado_a_medias_por_un_bug(
    ...
    original = chunking_impl.classify_chunk_categories
    llamadas = {"n": 0}
    ...
    monkeypatch.setattr(chunking_impl, "classify_chunk_categories", falla_en_el_segundo)
...
def test_un_bloque_con_datos_raros_sigue_sin_tumbar_el_pliego(
    ...
    monkeypatch.setattr(chunking_impl, "classify_chunk_categories", falla_en_el_primero)
...
def test_lo_que_se_saltea_queda_contado(monkeypatch: pytest.MonkeyPatch, caplog: Any) -> None:
    ...
    monkeypatch.setattr(chunking_impl, "classify_chunk_categories", falla_siempre_en_la_pagina_2)
