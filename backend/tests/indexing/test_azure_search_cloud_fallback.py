...
def _patch_search_client(monkeypatch: pytest.MonkeyPatch, client) -> None:
    monkeypatch.setattr(
        "infra.ports.azure_search._search_chunk_select_fields",
        lambda: [
            "id",
            "analysis_id",
            "document_id",
    ...
    monkeypatch.setattr("infra.ports.azure_search._embed_query_or_none", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
...
def test_fetch_all_analysis_chunks_no_usa_busqueda_vectorial(
    ...
    monkeypatch.setattr(
        "infra.ports.azure_search._embed_query_or_none", _embed_should_not_be_called
    )
...
def test_fetch_all_analysis_chunks_reporta_truncamiento(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import infra.ports.azure_search as azure_search_module
    from infra.ports.azure_search import fetch_all_analysis_chunks
