...
def test_el_ruido_sin_categoria_no_le_gana_a_la_caratula_real(
    ...
    resultado = chunk_retrieval._retrieve_with_category_priority(
...
def test_la_telemetria_reporta_sin_categoria_y_no_none(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(chunk_retrieval, "logger", _Logger())
    monkeypatch.setattr(
        chunk_retrieval,
        "search_hybrid",
    ...
    chunk_retrieval._retrieve_with_category_priority(
        query="identificación del procedimiento",
        analysis_id="analysis-1",
        top_k=1,
