...
def test_riesgo_item_subtipo_default():
    ...
    riesgo = RiesgoItem(
        ...
        confidence=0.7,
        source_references=[
            SourceReference(document_id="doc-123", page_number=10, citation="Texto de evidencia")
        ],
    )
...
def test_tipo_riesgo_enum_valores():
    ...
    expected_tipos = ["descalificacion", "penalizacion", "legal", "operativo", "financiero", "otro"]
...
def test_riesgo_item_metadata_opcional():
    ...
    riesgo = RiesgoItem(
        ...
        source_references=[
            SourceReference(
                document_id="doc-1", page_number=3, citation="Evidencia del riesgo operativo"
            )
        ],
    )
